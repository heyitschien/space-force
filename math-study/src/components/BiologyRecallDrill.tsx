...
import { RECALL_DRILL_ITEMS } from '../data/biologyFacts';

interface BiologyRecallDrillProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function BiologyRecallDrill({
  isOpen,
  onClose,
}: BiologyRecallDrillProps) {
  ...
  const handleAnswer = useCallback(
    (answer: string) => {
      ...
      if (currentIndex >= RECALL_DRILL_ITEMS.length - 1) {
        setPhase('done');
      } else {
        setTimeout(() => {
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl">
        ...
        {phase === 'ready' && (
          <div className="py-4">
            ...
            <p className="mt-3 text-slate-600">
              Answer 10 questions as fast as you can. You have 60 seconds.
              Target: 9+/10 quickly and confidently.
            </p>
          ...
          <div className="py-4">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-medium text-slate-500">
                {currentIndex + 1} / {RECALL_DRILL_ITEMS.length}
              </span>
              <span
            ...
            <div className="h-2 rounded-full bg-slate-200">
              <div
                ...
                style={{
                  width: `${(secondsLeft / DRILL_SECONDS) * 100}%`,
                }}
              />
            </div>
        ...
        {phase === 'done' && (
          <div className="py-6">
            ...
            <p className="mt-3 text-slate-600">
              You got {score} out of {RECALL_DRILL_ITEMS.length} correct.
            </p>
            {score >= 9 && (
              <p className="mt-2 text-sm font-medium text-emerald-600">
                Target met: 9+/10. Great recall!
              </p>
            )}
            {missedIds.length > 0 && (
              <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
                ...
                <ul className="mt-2 list-inside list-disc text-sm text-amber-700">
                  {missedIds.map((id) => {
                    const item = RECALL_DRILL_ITEMS.find((i) => i.id === id);
                    return item ? (
                      <li key={id}>
