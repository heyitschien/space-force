...
} from '../data/chemistryFacts';

interface ChemistryQuizModalProps {
  isOpen: boolean;
  onClose: () => void;
}

function shuffleQuestions(questions: ChemistryQuizQuestion[]): ChemistryQuizQuestion[] {
...
function buildQuestionSet(questions: ChemistryQuizQuestion[]): ChemistryQuizQuestion[] {
...
export function ChemistryQuizModal({ isOpen, onClose }: ChemistryQuizModalProps) {
  const totalQuestions = CHEMISTRY_QUIZ_QUESTIONS.length;
  ...
  const currentPool = useMemo(
    () =>
      retryMode
        ? CHEMISTRY_QUIZ_QUESTIONS.filter((q) => missedSet.has(q.id))
        : CHEMISTRY_QUIZ_QUESTIONS,
    [retryMode, missedSet]
  );
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl">
        ...
        {!isComplete && (
          <>
            <div className="mb-6">
              <div className="mb-2 flex items-center justify-between gap-4">
                <h2 className="text-2xl font-bold text-slate-800">
                  {retryMode
                    ? 'Retry Your Missed Chemistry Questions'
                    : 'Test Your Chemistry & Atoms Knowledge'}
                </h2>
              ...
              <p className="text-sm text-slate-500">
                {retryMode
                  ...
                  : 'ASVAB General Science chemistry and atom review'}
              </p>
            ...
            {showFeedback && (
              <div className="mt-6 rounded-xl border border-sky-100 bg-sky-50 p-5">
                <p
                  className={`font-bold ${
          ...
          <div className="py-6">
            ...
            {missedQuestionIds.length > 0 && (
              <p className="mt-2 text-sm text-slate-600">
                You reviewed {missedQuestionIds.length} missed concept
                {missedQuestionIds.length === 1 ? '' : 's'} in a retry round.
              </p>
            )}
            <p className="mt-2 text-sm text-slate-500">
              Review quick facts, then restart to lock concepts into memory.
            </p>
