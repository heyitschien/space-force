...
import { CHEMISTRY_RECALL_DRILL_ITEMS } from '../data/chemistryFacts';

interface ChemistryRecallDrillProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function ChemistryRecallDrill({ isOpen, onClose }: ChemistryRecallDrillProps) {
  ...
  const itemsWithShuffledOptions = useMemo(
    () =>
      CHEMISTRY_RECALL_DRILL_ITEMS.map((item) => ({
        ...item,
  ...
  const handleAnswer = useCallback(
    (answer: string) => {
      ...
      if (currentIndex >= CHEMISTRY_RECALL_DRILL_ITEMS.length - 1) {
        setPhase('done');
      } else {
        setTimeout(() => {
  ...
  return (
    ...
    >
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl">
        ...
        {phase === 'ready' && (
          <div className="py-4">
            <h2 className="text-2xl font-bold text-slate-800">60-Second Recall Drill</h2>
            <p className="mt-3 text-slate-600">
              Answer chemistry and atom questions fast. Target: 8/8 under 60 seconds.
            </p>
          ...
          <div className="py-4">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-medium text-slate-500">
                {currentIndex + 1} / {CHEMISTRY_RECALL_DRILL_ITEMS.length}
              </span>
              <span className={`text-lg font-bold ${secondsLeft <= 10 ? 'text-red-600' : 'text-slate-700'}`}>
                {secondsLeft}s
              </span>
            </div>
            ...
            <p className="mt-6 text-lg font-medium text-slate-700">{currentItem.question}</p>
        ...
        {phase === 'done' && (
          <div className="py-6">
            <h2 className="text-2xl font-bold text-slate-800">Drill complete</h2>
            <p className="mt-3 text-slate-600">
              You got {score} out of {CHEMISTRY_RECALL_DRILL_ITEMS.length} correct.
            </p>
            {score >= 7 && (
            ...
            {missedIds.length > 0 && (
              <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
                <h4 className="font-medium text-amber-800">Review these concepts:</h4>
                <ul className="mt-2 list-inside list-disc text-sm text-amber-700">
                  {missedIds.map((id) => {
                    const item = CHEMISTRY_RECALL_DRILL_ITEMS.find((entry) => entry.id === id);
                    return item ? (
                      <li key={id}>
