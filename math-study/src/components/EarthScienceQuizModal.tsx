...
import {
  ...
  type EarthScienceQuizQuestion,
} from '../data/earthScienceFacts';

interface EarthScienceQuizModalProps {
  isOpen: boolean;
  onClose: () => void;
}

function shuffleQuestions(
  questions: EarthScienceQuizQuestion[]
): EarthScienceQuizQuestion[] {
...
export function EarthScienceQuizModal({
  isOpen,
  onClose,
}: EarthScienceQuizModalProps) {
  const totalQuestions = EARTH_SCIENCE_QUIZ_QUESTIONS.length;
  ...
  const currentPool = useMemo(
    () =>
      retryMode
        ? EARTH_SCIENCE_QUIZ_QUESTIONS.filter((q) => missedSet.has(q.id))
        : EARTH_SCIENCE_QUIZ_QUESTIONS,
    [retryMode, missedSet]
  );
            ...
            <div className="mb-6">
              <div className="mb-2 flex items-center justify-between gap-4">
                <h2 className="text-2xl font-bold text-slate-800">
                  {retryMode
                    ? 'Retry Your Missed Earth Science Questions'
                    : 'Test Your Earth Science Knowledge'}
                </h2>
              ...
              <p className="text-sm text-slate-500">
                {retryMode
                  ...
                  : 'ASVAB General Science Earth Science review'}
              </p>
