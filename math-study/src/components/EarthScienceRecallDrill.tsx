...
import { EARTH_SCIENCE_RECALL_DRILL_ITEMS } from '../data/earthScienceFacts';

interface EarthScienceRecallDrillProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function EarthScienceRecallDrill({
  isOpen,
  onClose,
}: EarthScienceRecallDrillProps) {
  ...
  const itemsWithShuffledOptions = useMemo(
    () =>
      EARTH_SCIENCE_RECALL_DRILL_ITEMS.map((item) => ({
        ...item,
  ...
  const handleAnswer = useCallback(
    (answer: string) => {
      ...
      if (currentIndex >= EARTH_SCIENCE_RECALL_DRILL_ITEMS.length - 1) {
        setPhase('done');
      } else {
        setTimeout(() => {
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl">
        ...
        {phase === 'ready' && (
          <div className="py-4">
            ...
            <p className="mt-3 text-slate-600">
              Answer Earth Science questions fast. Target: 9/10+ under 60
              seconds.
            </p>
          ...
          <div className="py-4">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-medium text-slate-500">
                {currentIndex + 1} / {EARTH_SCIENCE_RECALL_DRILL_ITEMS.length}
              </span>
              <span
        ...
        {phase === 'done' && (
          <div className="py-6">
            ...
            <p className="mt-3 text-slate-600">
              You got {score} out of {EARTH_SCIENCE_RECALL_DRILL_ITEMS.length}{' '}
              correct.
            </p>
            {score >= 9 && (
            ...
            {missedIds.length > 0 && (
              <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
                ...
                <ul className="mt-2 list-inside list-disc text-sm text-amber-700">
                  {missedIds.map((id) => {
                    const item = EARTH_SCIENCE_RECALL_DRILL_ITEMS.find(
                      (entry) => entry.id === id
                    );
                    return item ? (
                      <li key={id}>
