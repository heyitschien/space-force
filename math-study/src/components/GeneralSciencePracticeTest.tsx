...
import { saveResult } from '../utils/testResults';
import {
  selectNextQuestion,
  getDifficulty,
  DIFFICULTY_POINTS,
...
interface GeneralSciencePracticeTestProps {
  onClose: () => void;
...
function getFirstMediumQuestion(): GsQuestion | null {
...
function getQuestionsForMode(mode: GeneralScienceTestMode): GsQuestion[] {
  if (mode === 'adaptive') {
...
export function GeneralSciencePracticeTest({
  onClose,
  mode = 'mix',
}: GeneralSciencePracticeTestProps) {
  const [phase, setPhase] = useState<'start' | 'running' | 'complete'>('start');
  const [questions, setQuestions] = useState<GsQuestion[]>([]);
    ...
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-50 p-4 md:p-8 font-sans">
        ...
        <div className="mx-auto flex min-h-[60vh] max-w-2xl flex-col items-center justify-center">
          ...
          <button
            onClick={startTest}
            className="rounded-xl bg-indigo-600 px-8 py-4 font-bold text-white shadow-lg transition-all hover:bg-indigo-500 active:scale-95"
          >
            Start Test
          </button>
        </div>
        ...
        <div className="mx-auto max-w-2xl overflow-hidden rounded-2xl bg-white shadow-xl">
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <button
                onClick={startTest}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-4 font-bold text-white shadow-lg transition-all hover:bg-indigo-500"
              >
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-100 p-4 md:p-8 font-sans">
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <progress
          className="mb-8 h-2 w-full overflow-hidden rounded-full [&::-moz-progress-bar]:bg-indigo-600 [&::-webkit-progress-bar]:bg-slate-200 [&::-webkit-progress-value]:bg-indigo-600"
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
          <div className="border-b border-slate-100 bg-slate-50 px-8 py-3">
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">
              {currentQuestion.bucket ?? 'General Science'}
            </span>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {currentQuestion.options.map((option) => {
                let buttonStyle = 'border-slate-200 hover:border-indigo-400 hover:bg-indigo-50';
                ...
                if (showFeedback) {
                  ...
                  buttonStyle = 'border-indigo-500 bg-indigo-50 ring-2 ring-indigo-100';
                }
          ...
          {showFeedback && (
            <div className="border-t border-slate-800 bg-slate-900 p-8 transition-opacity duration-300">
              ...
              <button
                onClick={handleNext}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-5 font-bold text-white shadow-lg transition-all hover:bg-indigo-500 active:scale-[0.98]"
              >
