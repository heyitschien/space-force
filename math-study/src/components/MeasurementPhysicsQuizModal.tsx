...
import {
  MEASUREMENT_PHYSICS_QUIZ_QUESTIONS,
  type MeasurementPhysicsQuizQuestion,
} from '../data/measurementPhysicsFacts';

interface MeasurementPhysicsQuizModalProps {
  isOpen: boolean;
  onClose: () => void;
}

function shuffleQuestions(
  questions: MeasurementPhysicsQuizQuestion[]
): MeasurementPhysicsQuizQuestion[] {
...
export function MeasurementPhysicsQuizModal({
  isOpen,
  onClose,
}: MeasurementPhysicsQuizModalProps) {
  const totalQuestions = MEASUREMENT_PHYSICS_QUIZ_QUESTIONS.length;
  ...
  const currentPool = useMemo(
    () =>
      retryMode
        ? MEASUREMENT_PHYSICS_QUIZ_QUESTIONS.filter((q) => missedSet.has(q.id))
        : MEASUREMENT_PHYSICS_QUIZ_QUESTIONS,
    [retryMode, missedSet]
  );
    ...
    >
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl">
            ...
            <div className="mb-6">
              <div className="mb-2 flex items-center justify-between gap-4">
                <h2 className="text-2xl font-bold text-slate-800">
                  {retryMode
                    ? 'Retry Your Missed Measurement & Physics Questions'
                    : 'Test Your Measurement & Physics Knowledge'}
                </h2>
              ...
              <p className="text-sm text-slate-500">
                {retryMode
                  ...
                  : 'ASVAB General Science Measurement & Physics review'}
              </p>
        ...
        {isComplete && (
          <div className="py-6">
            <h2 className="text-2xl font-bold text-slate-800">
              Measurement & Physics quiz complete
            </h2>
