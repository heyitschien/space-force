...
import pcData from '../data/paragraphComprehensionPracticeQuestions.json';
import { savePcResult, type ParagraphComprehensionTestMode } from '../utils/testResults';
import {
  selectNextQuestion,
  getDifficulty,
  DIFFICULTY_POINTS,
...
interface ParagraphComprehensionPracticeTestProps {
  onClose: () => void;
...
function getFirstMediumQuestion(): PcQuestion | null {
...
function getQuestionsForMode(mode: ParagraphComprehensionTestMode): PcQuestion[] {
  if (mode === 'adaptive') {
...
export function ParagraphComprehensionPracticeTest({
  onClose,
  mode = 'mix',
}: ParagraphComprehensionPracticeTestProps) {
    ...
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-50 p-4 md:p-8 font-sans">
        ...
        <div className="mx-auto flex min-h-[60vh] max-w-2xl flex-col items-center justify-center">
          ...
          <button
            onClick={startTest}
            className="rounded-xl bg-emerald-600 px-8 py-4 font-bold text-white shadow-lg transition-all hover:bg-emerald-500 active:scale-95"
          >
            Start Test
          </button>
        </div>
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-50 p-4 md:p-8 font-sans">
        ...
        <div className="mx-auto max-w-2xl overflow-hidden rounded-2xl bg-white shadow-xl">
          <div className="bg-emerald-900 p-8 text-center text-white">
            ...
            <p className="font-medium text-emerald-200">Paragraph Comprehension — ASVAB Prep</p>
          </div>
          <div className="p-8">
            {mode === 'adaptive' && maxWeighted != null && (
              <div className="mb-6 rounded-xl border-2 border-emerald-200 bg-emerald-50/50 p-4">
                  ...
                  {missedQuestions.map(({ question, selectedAnswer }) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mb-2 max-h-24 overflow-y-auto text-sm text-slate-600">
                          {question.passage}
                        </p>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <button
                onClick={startTest}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 py-4 font-bold text-white shadow-lg transition-all hover:bg-emerald-500"
              >
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-100 p-4 md:p-8 font-sans">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <progress
          className="mb-8 h-2 w-full overflow-hidden rounded-full [&::-moz-progress-bar]:bg-emerald-600 [&::-webkit-progress-bar]:bg-slate-200 [&::-webkit-progress-value]:bg-emerald-600"
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
          ...
          <div className="p-6 md:p-8">
            <div className="mb-6 max-h-[min(40vh,22rem)] overflow-y-auto rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 text-base leading-relaxed text-slate-800 shadow-inner">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-emerald-800">
                Passage
              </p>
              <p className="whitespace-pre-wrap">{currentQuestion.passage}</p>
            </div>
            <p className="mb-6 text-xl font-semibold leading-relaxed text-slate-800 md:text-2xl">
              {currentQuestion.text}
            </p>

            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {currentQuestion.options.map((option) => {
                let buttonStyle = 'border-slate-200 hover:border-emerald-400 hover:bg-emerald-50';
                ...
                if (showFeedback) {
                  ...
                  buttonStyle = 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-100';
                }
                ...
                return (
                  <button
                    ...
                    className={`flex items-start justify-between gap-3 rounded-xl border-2 p-4 text-left transition-all duration-200 md:p-6 ${buttonStyle}`}
                  >
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <span className="min-w-0 text-base font-bold leading-snug text-slate-800 md:text-lg">
                        {option.text}
                      </span>
                    </div>
                    {icon}
          ...
          {showFeedback && (
            <div className="border-t border-slate-800 bg-slate-900 p-8 transition-opacity duration-300">
              ...
              <button
                type="button"
                onClick={handleNext}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 py-5 font-bold text-white shadow-lg transition-all hover:bg-emerald-500 active:scale-[0.98]"
              >
