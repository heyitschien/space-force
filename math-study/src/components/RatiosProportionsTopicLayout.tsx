...
import {
  RATIOS_INTRO,
  RATIOS_CORE_PRINCIPLE,
  RATIOS_PATTERNS,
  RATIOS_WORKED_EXAMPLES,
  RATIOS_MENTAL_MODELS,
  RATIOS_DRILL_SETS,
  RATIOS_STRATEGY,
  RATIOS_FINAL_FRAMEWORK,
  type SolvedProblem,
} from '../data/ratiosProportionsContent';
...
interface RatiosProportionsTopicLayoutProps {
  topic: ArTopic;
}
...
export function RatiosProportionsTopicLayout({ topic }: RatiosProportionsTopicLayoutProps) {
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <section className="mb-10 rounded-2xl border border-rose-200 bg-white p-6 shadow-sm">
          <h2 className="mb-3 text-2xl font-extrabold text-rose-800">
            {RATIOS_INTRO.headline}
          </h2>
          <p className="mb-2 text-sm font-semibold text-slate-600">{RATIOS_INTRO.tagline}</p>
          <p className="mb-4 text-slate-700">{RATIOS_INTRO.insight}</p>
          <ul className="grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
            {RATIOS_INTRO.goals.map((goal) => (
          ...
          {memoryDeck.length > 0 && (
            <div className="mt-5 rounded-xl border border-indigo-200 bg-indigo-50/60 p-4">
              <p className="text-sm font-semibold text-indigo-900">
                Start here for today: lock in ratio facts and rules first.
              </p>
        ...
        {memoryDeck.length > 0 && (
          <section id="think-about-it-memory-game" className="mb-10">
            ...
            <p className="mb-4 text-sm text-slate-600">
              ...
              feedback to strengthen long-term memory of ratio facts and rules.
            </p>
        ...
        <section className="mb-10">
          <SectionHeading>{RATIOS_CORE_PRINCIPLE.title}</SectionHeading>
          <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div>
              <p className="mb-2 text-lg font-semibold text-slate-800">💡 What is a Ratio?</p>
              <p className="mb-2 text-slate-700">
                A <strong>ratio</strong> = {RATIOS_CORE_PRINCIPLE.ratioDefinition}
              </p>
              <p className="mb-2 text-sm text-slate-600">👉 Think: &quot;{RATIOS_CORE_PRINCIPLE.ratioThink}&quot;</p>
              <ul className="list-inside list-disc text-sm text-slate-700">
                {RATIOS_CORE_PRINCIPLE.ratioExamples.map((ex) => (
                  <li key={ex}>{ex}</li>
                ))}
              </ul>
            </div>
            <div>
              <p className="mb-2 text-lg font-semibold text-slate-800">💡 What is a Proportion?</p>
              <p className="mb-2 text-slate-700">
                A <strong>proportion</strong> = {RATIOS_CORE_PRINCIPLE.proportionDefinition}
              </p>
              <p className="mb-2 text-sm text-slate-600">
                👉 Think: &quot;{RATIOS_CORE_PRINCIPLE.proportionThink}&quot;
              </p>
              <FormulaBlock>{RATIOS_CORE_PRINCIPLE.proportionExample}</FormulaBlock>
            </div>
            <div className="rounded-xl border border-rose-200 bg-rose-50 p-4">
              <p className="text-lg font-bold text-rose-800">
                👉 {RATIOS_CORE_PRINCIPLE.goldenRule}
              </p>
              <p className="mt-1 text-slate-700">{RATIOS_CORE_PRINCIPLE.goldenRuleDetail}</p>
            </div>
          </div>
        </section>

        <section className="mb-10">
          <SectionHeading>🔥 PART 2 — 3 CORE ASVAB PATTERNS</SectionHeading>
          <p className="mb-4 text-sm text-slate-600">
            You will see ONLY these 3 patterns (disguised):
          </p>
          <div className="space-y-4">
            {RATIOS_PATTERNS.map((pattern) => (
              <div
                key={pattern.title}
                className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"
              >
                <p className="mb-2 text-lg font-bold text-slate-800">{pattern.title}</p>
                <p className="mb-2 text-sm text-rose-700">👉 {pattern.cue}</p>
                <p className="text-sm text-slate-600">Example: {pattern.example}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-10">
          <SectionHeading>⚔️ PART 3 — WORKED EXAMPLES (STEP-BY-STEP)</SectionHeading>
          <div className="space-y-4">
            {RATIOS_WORKED_EXAMPLES.map((problem) => (
        ...
        <section className="mb-10">
          <SectionHeading>🧠 PART 4 — MENTAL MODELS (ELITE UNDERSTANDING)</SectionHeading>
          <div className="space-y-4">
            {RATIOS_MENTAL_MODELS.map((model) => (
              <div
                key={model.title}
                className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"
              >
                ...
                {model.note && (
                  <p className="mt-2 text-xs italic text-slate-600">{model.note}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <section className="mb-10">
          <SectionHeading>🏋️ PART 5 — DRILLS (BUILD SPEED)</SectionHeading>
          {RATIOS_DRILL_SETS.map((set, setIdx) => (
            <div
              key={set.title}
                          ...
                          <ol className="mt-3 space-y-2 text-sm text-slate-700">
                            {item.steps.map((s, idx) => (
                              <li key={idx} className="rounded-md border border-slate-100 bg-white px-2 py-1.5">
                                <p className="font-mono text-rose-700">{s.step}</p>
                                <p className="mt-0.5 text-xs italic text-slate-600">Why: {s.why}</p>
                              </li>
                            ))}
                          </ol>
        ...
        <section className="mb-10">
          <SectionHeading>{RATIOS_STRATEGY.title}</SectionHeading>
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="mb-4 text-slate-700">{RATIOS_STRATEGY.insight}</p>
            <p className="mb-2 text-sm font-semibold text-slate-800">⚡ FAST METHOD:</p>
            <ol className="mb-4 list-inside list-decimal space-y-1 text-sm text-slate-700">
              {RATIOS_STRATEGY.fastMethod.map((step) => (
                <li key={step}>{step}</li>
              ))}
            </ol>
            <p className="mb-2 text-sm font-semibold text-slate-800">🚨 COMMON TRAPS:</p>
            <ul className="space-y-1 text-sm text-slate-700">
              {RATIOS_STRATEGY.commonTraps.map((trap) => (
                <li key={trap}>{trap}</li>
              ))}
            </ul>
          </div>
        ...
        <section className="mb-10 rounded-2xl border border-rose-200 bg-white p-6 shadow-sm">
          <SectionHeading>{RATIOS_FINAL_FRAMEWORK.title}</SectionHeading>
          <p className="mb-4 text-sm text-slate-600">
            When you see a word problem:
          </p>
          <div className="space-y-3">
            {RATIOS_FINAL_FRAMEWORK.steps.map((s) => (
              <div key={s.step} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                <p className="font-semibold text-rose-800">{s.step}</p>
                <p className="text-sm text-slate-700">👉 {s.question}</p>
              </div>
            ))}
          </div>
          <p className="mt-4 font-semibold text-rose-800">
            {RATIOS_FINAL_FRAMEWORK.closing}
          </p>
        </section>
